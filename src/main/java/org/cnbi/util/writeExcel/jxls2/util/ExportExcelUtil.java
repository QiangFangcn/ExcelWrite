...
import org.cnbi.util.writeExcel.jxls2.commandfordata.DataHandler2_0;
import org.cnbi.util.writeExcel.jxls2.commandfordata.IsComboCompanySignHandler;
import org.cnbi.util.writeExcel.jxls2.commandfordata.ProcessDataForDataGrid;
...
//        Transformer poiTransformer = PoiTransformer.createSxssfTransformer(workbook, 100000, false, true); // 似乎在这种模板中无法使用
        ...
        try {
            ...
            XlsCommentAreaBuilder.addCommandMapping("dataHandler", DataHandler.class);
            XlsCommentAreaBuilder.addCommandMapping("isComboCompanySignHandler", IsComboCompanySignHandler.class);
            ...
            XlsCommentAreaBuilder.addCommandMapping("processItemsForGrid", ProcessDataForDataGrid.class);
            XlsCommentAreaBuilder.addCommandMapping("dataHandler2", DataHandler2_0.class);
...
//                        context.toMap().clear();
                    ...
                    * 为了一张表做的特殊处理
                    * */
                    if(context.toMap().containsKey("header")) {
                        GridCommand gridCommand = (GridCommand) xlsArea.getCommandDataList().get(1).getCommand();
                        StringBuilder sb = new StringBuilder();
                        sb.append("客户,");
                        List<Map> list = (List) context.getVar("header");
                        for (Map map : list) {
                            String value = ((String) map.get("value")).replaceAll("\\(", "").replaceAll("\\)", "");
